/*
bxSlider v2.0
Plugin developed by: Steven WandFerski
http://bxslider.com
...
if(o.auto){clearInterval(t);$this.parent().siblings('.auto_controls').find('a').html(o.start_text);playing=false;}
return false;});}
if(o.auto&&!o.ticker){t=setInterval(function(){var pageNum = $(".bx_pager").children("a").length; if(o.auto_direction=='next'){if(pageNum>1){move_slide(++current);}}else{if(pageNum>1){move_slide(--current);}}},o.pause);if(o.auto_hover){$this.hover(function(){clearInterval(t);},function(){if(playing){t=setInterval(function(){var pageNum = $(".bx_pager").children("a").length;if(o.auto_direction=='next'){if (pageNum > 1) {move_slide(++current);}}
else{if (pageNum > 1) {move_slide(--current);}}},o.pause);}});}
